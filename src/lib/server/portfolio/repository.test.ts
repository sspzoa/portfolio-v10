...
import { getSectionErrorMessage, PortfolioValidationError } from "~/lib/server/portfolio/errors";
import {
  fetchAboutMe,
  fetchCareers,
  fetchEducation,
...
describe("Portfolio repository", () => {
  ...
  ] as const)("loads %s without a website property", async (_, fetchItems) => {
    mockResponse({
      results: [timelinePage()],
      has_more: false,
      next_cursor: null,
    });
