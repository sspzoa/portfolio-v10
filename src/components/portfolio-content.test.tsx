...
import { portfolioSources } from "~/lib/server/portfolio/sources";
...
test("returns complete readable HTML, including collapsed projects, despite one failed source", async () => {
  ...
  const request = spyOn(globalThis, "fetch").mockImplementation(
    Object.assign(
      async (input: Parameters<typeof fetch>[0]) => {
        ...
        if (url.includes(portfolioSources.careers.id))
            ...
            ? [false, true].map((side) => ({
                ...
                properties: {
                  name: {
                    title: [
                      {
                        plain_text: side ? "접힌 프로젝트" : "주요 프로젝트",
                      },
                    ],
                  },
                  shortDescription: {
  ...
  try {
    ...
    const html = renderToString(() => <PortfolioContent data={data} />);
