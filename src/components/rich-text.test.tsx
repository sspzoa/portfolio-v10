...
describe("Notion rich text presentation", () => {
  ...
  test("keeps nested lists and standard hard line breaks in server HTML", () => {
    const html = renderToString(() => <RichText>{"첫 줄  \n다음 줄\n\n- 주요 작업\n  - 세부 작업"}</RichText>);
    expect(html).toMatch(/첫 줄<br\s*\/?>(?:\n)다음 줄/);
  ...
  test.each([
    ...
    const html = renderToString(() => <RichText>{`[**링크 이름**](${url})`}</RichText>);
  ...
  test("renders CMS headings as separate paragraphs without changing page hierarchy", () => {
    const html = renderToString(() => <RichText>{"# 제목\n\n## 소제목\n\n###### 참고"}</RichText>);
